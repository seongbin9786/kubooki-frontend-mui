...
class SocialSessionPreProcessor {
  constructor() {
    ...
    this._logoutMapping.set(FACEBOOK, this.logoutFacebook);
  ...
  preProcessLogin = type => {
    ...
    throw Error('지원하지 않는 방식의 소셜 로그인입니다: ' + type);
  }
  ...
  preProcessLogout = type => {
    ...
    throw Error('지원하지 않는 방식의 소셜 로그아웃입니다: ' + type);
  }
  ...
  loginNaver = () => new Promise(resolve => {
    ...
    if (loginStatus.status) { // 로그인돼있음
      const { accessToken: { accessToken } } = loginStatus;
  ...
  logoutFacebook = () => window.FB.logout();
